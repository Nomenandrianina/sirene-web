...
export function AudioAlerteBngrcUploadForm({ initialData, onSubmit, loading, error }: Props) {
  ...
  // Sirènes — utiles uniquement en mode édition
  ...
  const filteredTypes = useMemo(() =>
    form.alerteBngrcId
      ? allTypes.filter((t: any) => Number(t.alerteBngrcId) === form.alerteBngrcId)
      : allTypes,
    [allTypes, form.alerteBngrcId]);

  const filteredCats = useMemo(() =>
    form.typeAlerteBngrcId
      ? allCats.filter((c: any) => Number(c.typeAlerteBngrcId) === form.typeAlerteBngrcId)
      : allCats,
    [allCats, form.typeAlerteBngrcId]);

  /**
   * En mode édition : exclure la catégorie d'origine du blocage
   * pour ne pas verrouiller le formulaire sur lui-même.
   */
  const effectiveUsedIds = useMemo(() =>
    isEdit
      ? (usedCatIds as number[]).filter(id => id !== initialData?.categorieAlerteBngrcId)
      : (usedCatIds as number[]),
    [usedCatIds, isEdit, initialData?.categorieAlerteBngrcId]);

  function isCatUsed(catId: number): boolean {
    return (effectiveUsedIds as number[]).includes(catId);
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // En création : sireneIds vide → backend associe toutes les sirènes
    // En édition  : on garde la sélection de l'utilisateur (form.sireneIds)
    await onSubmit(
      isEdit ? form : { ...form, sireneIds: [] },
      file ?? undefined,
    );
  }
  ...
  const isValid =
    ...
    !isCatUsed(form.categorieAlerteBngrcId) &&
    (isEdit || !!file);
  ...
  return (
    <div className="sirene-form-page">
      ...
      <form onSubmit={handleSubmit} className="sirene-form-layout">
        ...
        <div className="sirene-form-card">
          ...
          <div className="sirene-fields-grid">
            ...
            <div className="sirene-field" style={{ gridColumn: "1/-1" }}>
              ...
              <select
                required
                disabled={!form.typeAlerteBngrcId}
              ...
              {form.categorieAlerteBngrcId > 0 && isCatUsed(form.categorieAlerteBngrcId) && (
        ...
        <div className="sirene-form-card">
          {isEdit ? (
            /* Édition : sélection manuelle des sirènes */
            <>
            ...
            /* Création : toutes les sirènes automatiquement */
