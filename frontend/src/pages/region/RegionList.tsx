...
import {
  ...
  Loader2, ChevronLeft, ChevronRight, Map,
} from "lucide-react";
    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal-box modal-sm" onClick={e => e.stopPropagation()}>
        <div className="modal-header">
          <h2 className="modal-title">Supprimer la région</h2>
          <button className="modal-close" onClick={onClose}><X size={18} /></button>
        </div>
        <div className="modal-body">
          <p className="confirm-text">
            Voulez-vous vraiment supprimer la région <strong>{name}</strong> ?
...
export default function RegionList() {
  ...
  const regions: Region[] = Array.isArray(raw)
    ? raw
  ...
  const filtered = useMemo(() => {
    ...
    return regions.filter(r =>
      ...
      (r as any).province?.name?.toLowerCase().includes(q)
    );
  }, [regions, search]);
  ...
  const deleteMut = useMutation({
    mutationFn: (id: number) => regionsApi.remove(id),
    onSuccess: () => {
  ...
  return (
    <AppLayout>
      <div className="page-wrap">
        ...
        <div className="panel">
          <div className="panel-header">
            <span className="panel-title">Liste des régions</span>
          ...
          <div style={{ overflowX: "auto" }}>
            {isLoading ? (
              ...
              <div className="empty-state">
                <Map size={28} /><p>Aucune région trouvée</p>
              </div>
            ) : (
                ...
                <tbody>
                  {paginated.map((r, i) => (
                    <tr key={r.id}>
                      ...
                      <td>
                        {(r as any).province?.name ? (
                          <span className="perm-tag">{(r as any).province.name}</span>
                        ) : (
      ...
      {delRegion && (
        <ConfirmModal
          name={delRegion.name}
          onClose={() => setDelRegion(null)}
          onConfirm={() => deleteMut.mutate(delRegion.id)}
