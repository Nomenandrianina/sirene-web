...
export default function SireneMap() {
  ...
  useEffect(() => {
    ...
    import("leaflet").then((L: any) => {
      ...
      // Bounding box approximative d'Antananarivo (ajuste si besoin)
      const antananarivoBounds = L.latLngBounds(
        [-19.05, 47.35], // sud-ouest
        [-18.75, 47.65]  // nord-est
      );
    
      const map = L.map(mapRef.current, {
        center: [-18.8792, 47.5079], // Antananarivo centre
        zoom: 12,
        minZoom: 11,
        maxZoom: 19,
        maxBounds: antananarivoBounds,
        maxBoundsViscosity: 1.0, // empêche complètement de sortir de la zone
        zoomControl: false,
      });
...
//---------------------------------- Ancien code centré sur madagascar -------------------------------------------------
