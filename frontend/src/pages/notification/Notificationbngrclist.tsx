...
import { NotificationBngrc, NotificationBngrcFilters }  from "@/types/notificationBngrc";
...
import { Search, Trash2, Bell, Filter, X, CheckCircle, Clock, XCircle, HelpCircle, Radio, Calendar, ChevronDown, AlertTriangle, MapPin, Layers, Wind,} from "lucide-react";
...
// ── Mini lecteur audio BNGRC ──────────────────────────────────────────────────

function AudioBngrcPlayer({ audio }: { audio: NotificationBngrc["audioBngrc"] }) {
  ...
  return (
    <div style={{
      ...
      borderRadius: 8, padding: "6px 10px", marginTop: 8, marginBottom: 6,
      transition: "all 0.2s",
    }}>
...
// ── Carte notification BNGRC ──────────────────────────────────────────────────

function NotifBngrcCard({ n, onDelete }: { n: NotificationBngrc; onDelete: () => void }) {
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap", marginBottom: 6 }}>
            ...
            {/* Type / Aléa */}
            {n.categorieAlerteBngrc?.type && (
              <Chip icon={<Wind size={9} />} label={n.categorieAlerteBngrc.type.name} color="#7c3aed" bg="#f5f3ff" />
            )}

            {/* Catégorie BNGRC — remplace sous-catégorie */}
            {n.categorieAlerteBngrc && (
              <Chip icon={<Layers size={9} />} label={n.categorieAlerteBngrc.name} color="#0f766e" bg="#f0fdfa" />
            )}

            {/* Fallback sur le champ type si pas de relation */}
            {!n.categorieAlerteBngrc && n.type && (
              <Chip icon={<Layers size={9} />} label={n.type} color="#0f766e" bg="#f0fdfa" />
            )}
          </div>

          {/* MiniPlayer audio BNGRC */}
          {n.audioBngrc && <AudioBngrcPlayer audio={n.audioBngrc} />}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
            {n.sirene && (
              <span style={{
                ...
                <Radio size={9} />
                {n.sirene.name ?? n.sirene.imei ?? `Sirène #${n.sireneId}`}
              </span>
            )}
            {zone !== "—" && (
      ...
      {expanded && (
        <div style={{
          ...
          {n.sirene?.name && <InfoBlock label="sirène" value={n.sirene.name} mono />}
          {n.categorieAlerteBngrc && <InfoBlock label="Catégorie " value={n.categorieAlerteBngrc.name} />}
          {n.categorieAlerteBngrc?.type && <InfoBlock label="Type / Aléa" value={n.categorieAlerteBngrc.type.name} />}
          {/* { n.operator &&  <InfoBlock label="Opérateur" value={n.operator} />} */}
          {n.observation && <InfoBlock label="Observation" value={n.observation} />}
          {n.user && <InfoBlock label="Envoyé par" value={`${n.user.first_name} ${n.user.last_name}`} />}
        </div>
      )}
    </div>
  );
...
export default function NotificationBngrcList() {
  const qc = useQueryClient();

  const [filters,     setFilters]     = useState<NotificationBngrcFilters>({ page: 1, limit: PER_PAGE });
  ...
  const { data: raw, isLoading } = useQuery({
    queryKey: ["notifications-bngrc", filters],
    queryFn:  () => notificationsBngrcApi.getAll(filters),
  });

//   const { data: statsRaw } = useQuery({
//     queryKey: ["notifications-bngrc-stats", filters],
//     queryFn:  () => notificationsBngrcApi.getStats(filters),
//   });
  ...
  const result: { data: NotificationBngrc[]; total: number; page: number; lastPage: number } =
    (raw as any)?.data
      ? raw as any
...
//   const stats   = statsRaw as any;
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter(n =>
      ...
      n.sirene?.name?.toLowerCase().includes(q) ||
      n.categorieAlerteBngrc?.name?.toLowerCase().includes(q) ||
      n.categorieAlerteBngrc?.type?.name?.toLowerCase().includes(q) ||
      n.audioBngrc?.mobileId?.toLowerCase().includes(q)
    );
  }, [items, search]);

  // ── Filtres ────────────────────────────────────────────────────────────────
  function updateFilter(key: keyof NotificationBngrcFilters, value: any) {
  ...
  const activeFilterCount = [
    filters.sireneId, filters.status, filters.startDate, filters.categorieAlerteBngrcId,
  ].filter(Boolean).length;

  // ── Suppression ────────────────────────────────────────────────────────────
  const deleteMut = useMutation({
    mutationFn: (id: number) => notificationsBngrcApi.remove(id),
    onSuccess:  () => {
      qc.invalidateQueries({ queryKey: ["notifications-bngrc"] });
      qc.invalidateQueries({ queryKey: ["notifications-bngrc-stats"] });
    ...
    <AppLayout>
      <div className="page-wrap">
        ...
        <div className="page-header">
          <div>
            <h1 className="text-xl font-semibold text-slate-900">Diffusions des alertes</h1>
            <p className="page-subtitle">Historique des alertes envoyées aux sirènes</p>
          </div>
        </div>
        ...
        <div className="panel">
            ...
            <div style={{ padding: "16px", borderBottom: "1px solid #f1f5f9", background: "#f8fafc" }}>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 12 }}>
                ...
                <div className="sirene-field">
                  <label>Sirène</label>
                  <select value={filters.sireneId ?? ""} onChange={e => updateFilter("sireneId", e.target.value ? +e.target.value : undefined)}>
          ...
          {activeFilterCount > 0 && (
            <div style={{ display: "flex", gap: 6, padding: "8px 16px", flexWrap: "wrap", borderBottom: "1px solid #f1f5f9" }}>
              ...
              {filters.categorieAlerteBngrcId && <FilterTag label={`Catégorie #${filters.categorieAlerteBngrcId}`} onRemove={() => setFilters(f => ({ ...f, categorieAlerteBngrcId: undefined, page: 1 }))} />}
          ...
          {!isLoading && result.lastPage > 1 && (
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 18px", borderTop: "1px solid #f1f5f9" }}>
              <span style={{ fontSize: 12, color: "#94a3b8" }}>
                Page {result.page} / {result.lastPage} — {result.total} diffusions
              </span>
