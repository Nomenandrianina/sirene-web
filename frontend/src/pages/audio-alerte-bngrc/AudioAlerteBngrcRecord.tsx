...
import { RecorderWidget }                              from "@/components/alerteaudio/Recorderwidget"; // réutilisé tel quel
import { ChevronLeft, Loader2, Mic, Check, Radio }     from "lucide-react";
...
function toArr(raw: any): any[] {
  ...
  function SireneCard({ sirene, checked, onToggle }: {
    sirene: any; checked: boolean; onToggle: (id: number) => void;
  }) {
    return (
      <button type="button" onClick={() => onToggle(sirene.id)}
        className={[
          "flex items-center gap-2 px-3 py-2 rounded-lg border text-sm transition text-left w-full",
          checked ? "bg-sky-50 border-sky-300 text-sky-800 font-medium" : "bg-white border-slate-200 text-slate-700 hover:border-slate-300",
        ].join(" ")}>
  ...
  export default function AudioAlerteBngrcRecord() {
    ...
    function toggleSirene(id: number) {
      setSireneIds(ids => ids.includes(id) ? ids.filter(x => x !== id) : [...ids, id]);
    }
    ...
    const createMut = useMutation({
      mutationFn: () => {
        ...
        fd.append("sireneIds",              JSON.stringify(sireneIds));
        if (name)        fd.append("name",        name);
        if (description) fd.append("description", description);
        fd.append("duration", String(recordedDuration));
        // mobileId NON envoyé — généré automatiquement backend
    ...
    function handleRecorded(file: File, duration: number) {
      ...
      // mobileId généré automatiquement côté backend — pas besoin de le pré-remplir
    }
              ...
              <>
                ...
                <SectionCard title="Classification BNGRC">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <Field label="Catégorie" required>
                      <select value={categorieId || ""} disabled={!typeId} className={inputCls}
                ...
                <SectionCard title="Sirènes de destination *">
                  {sirenes.length === 0 ? (
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                      {sirenes.map((s: any) => (
                        <SireneCard key={s.id} sirene={s} checked={sireneIds.includes(s.id)} onToggle={toggleSirene} />
                      ))}
                    </div>
                  )}
                  {sireneIds.length > 0 && (
                ...
                <div className="flex items-center justify-end gap-3 pb-8">
                  ...
                  <button type="button"
                    ...
                    Créer l'audio BNGRC{sireneIds.length > 1 ? ` (${sireneIds.length} sirènes)` : ""}
                  </button>
                </div>
              </>
            )}
