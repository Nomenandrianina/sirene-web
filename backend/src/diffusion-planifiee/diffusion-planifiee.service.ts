...
export class DiffusionPlanifieeService {
  ...
  async getClientPlanning(params: {
    customerId: number; sireneId: number; souscriptionId: number; from: string; to: string;
  ...
  async clientAutoGenerate(
    dto: ClientAutoGenerateDto & { sireneId: number },
    ...
    while (cursor <= endDate) {
      ...
      const dateStr       = toDateStr(cursor);
      const jourAutorise  = !joursAutorises.length || joursAutorises.includes(String(jourISO));
